...
def map():
    ...
    bounced = False
    for i in range(len(MAP["CollisionsList"])):
        if MAP["CollisionsList"][i].collidepoint(MAP["PlayerPos"]) == True:
            bounced = True

    if bounced == True:
        MAP["PlayerSpeed"][0] = -MAP["PlayerSpeed"][0] * 0.1
        MAP["PlayerPos"][0] += MAP["PlayerSpeed"][0] * frameTime * 30
    ...
    if bounced == True:
        MAP["PlayerSpeed"][1] = -MAP["PlayerSpeed"][1] * 0.1
        MAP["PlayerPos"][1] += MAP["PlayerSpeed"][1] * frameTime * 30
